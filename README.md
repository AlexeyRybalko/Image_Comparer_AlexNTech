...
- масштабирование, перемещение, mini-map и экспорт текущего вида
- светлую и тёмную темы

### Запуск

1. Откройте `ImageComparer.exe`.
2. Не удаляйте и не перемещайте папку `assets` отдельно от приложения.

### Основные возможности

- Сравнение двух изображений с вертикальным divider в режиме `Slide`
...
- Левый клик: поставить divider под курсор и сразу начать его двигать
- Правая кнопка мыши: перемещение текущего вида
- Колесо мыши: увеличение и уменьшение
- Двойная работа divider не нужна: он сразу ставится и двигается одним действием

#### В режиме Side-by-side

- Колесо мыши: масштабирует только ту сторону, над которой находится курсор
- Правая кнопка мыши: двигает левое или правое изображение отдельно
- Центральная линия визуально разделяет стороны и не даёт изображениям пересекаться
...
- поддерживает прокрутку нижним scrubber-слайдером
